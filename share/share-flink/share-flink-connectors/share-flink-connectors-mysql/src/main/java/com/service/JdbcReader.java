...
public class JdbcReader extends RichSourceFunction<StudentDemo> {
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        connection = DriverManager.getConnection(config.getString("jdbc.url"), config.getString("jdbc.username"), config.getString("jdbc.password"));//获取连接
        String sql = "select stuid,stuname,stuaddr,stusex from Student;";
        ps = connection.prepareStatement(sql);
    }

    //执行查询并获取结果
    public void run(SourceContext<StudentDemo> ctx) throws SQLException {
        try {
            ...
            while (resultSet.next()) {
                ...
                StudentDemo student = new StudentDemo(stuid, stuname, stuaddr, stusex);
                ctx.collect(student);//发送结果，结果是tuple2类型，2表示两个元素，可根据实际情况选择
            }
        } catch (Exception e) {
